...
<<<<<<< HEAD
  ...
  const SYSTEM_SCHEMA = [
    { path: 'state.sensory.ais_voltage_v', type: 'float', keywords: ['ais', 'v', 'voltage', 'power'] },
    { path: 'state.sensory.lantern_voltage_v', type: 'float', keywords: ['lantern', 'main', 'batt', 'v', 'voltage', 'power', 'local'] },
    { path: 'state.sensory.racon_voltage_v', type: 'float', keywords: ['racon', 'v', 'voltage', 'power'] },
    { path: 'state.sensory.lamp_current_a', type: 'float', keywords: ['lamp', 'current', 'a', 'amp'] },
    { path: 'state.sensory.temperature_c', type: 'float', keywords: ['temp', 'temperature', 'c', 'system'] },
    { path: 'state.sensory.door_open', type: 'bool', keywords: ['door', 'open', 'status'] },
    { path: 'state.sensory.ldr_status', type: 'bool', keywords: ['ldr', 'dark', 'light', 'ambient'] },
    { path: 'state.sc.voltage_v', type: 'float', keywords: ['sc', 'sc35', 'modbus', 'voltage', 'v'] },
    { path: 'state.sc.temperature_c', type: 'float', keywords: ['sc', 'sc35', 'temp', 'temperature'] },
    { path: 'state.ais.com_status', type: 'int', keywords: ['ais', 'comm', 'com', 'status', 'connection'] },
    { path: 'state.ais.query_lock_achieved', type: 'bool', keywords: ['ais', 'query', 'lock', 'stat', 'config'] },
    { path: 'state.ais.message_count', type: 'int', keywords: ['ais', 'msg', 'message', 'count', 'rx'] },
    { path: 'state.ais.latitude', type: 'float', keywords: ['lat', 'latitude', 'gps'] },
    { path: 'state.ais.longitude', type: 'float', keywords: ['lon', 'longitude', 'gps'] },
    { path: 'state.ais.firmware_version', type: 'string', keywords: ['ais', 'firmware', 'ver', 'version'] },
    { path: 'state.ais.serial_number', type: 'string', keywords: ['ais', 'serial', 'sn', 's/n'] },
    { path: 'state.lh.lhi_board_sn', type: 'string', keywords: ['lh', 'lhi', 'board', 'serial', 'sn', 's/n'] },
    { path: 'state.lh.lhi_firmware_ver', type: 'string', keywords: ['lh', 'lhi', 'firmware', 'ver', 'version'] },
    { path: 'state.ais.atonbit_i.bits.light', type: 'int', keywords: ['aton', 'light', 'status', 'pri', 'sec'] },
    { path: 'state.ais.atonbit_i.bits.racon', type: 'int', keywords: ['aton', 'racon', 'status'] },
    { path: 'state.ais.atonbit_i.bits.health', type: 'int', keywords: ['aton', 'health', 'stat'] },
    { path: 'state.config.lantern_monitor', type: 'int', keywords: ['config', 'lantern', 'monitor', 'mode'] },
    { path: 'state.config.racon_monitor', type: 'int', keywords: ['config', 'racon', 'monitor', 'mode'] },
    { path: 'state.last_master_update_s', type: 'int', keywords: ['clock', 'runtime', 'time', 'update'] },
    { path: 'state.last_vdo21_update_s', type: 'int', keywords: ['m21', 'vdo', 'ago', 'update'] },
    { path: 'state.last_abm06_update_s', type: 'int', keywords: ['m06', 'abm', 'ago', 'update'] }
  ];

  const processAIImport = (secId) => {
    const text = document.getElementById(`ai-paste-${secId}`).value;
    if (!text) return;
    ...
    lines.forEach(line => {
      // Matches formats like "2, Lantern V" or "Type 2 - Lantern V"
      const match = line.match(/(?:type\s*)?(\d)[\s,.-]+(.*)/i);
      if (match) {
        const typeId = parseInt(match[1]);
        const rawLabel = match[2].trim();
        
        // 1. Tokenize the label for heuristic matching
        const tokens = rawLabel.toLowerCase().replace(/[^a-z0-9]/g, ' ').split(' ').filter(Boolean);
        
        // 2. Score the schema
        let bestScore = -1;
        let bestMatch = null;

        SYSTEM_SCHEMA.forEach(schema => {
          let score = 0;
          const pathTokens = schema.path.toLowerCase().replace(/[^a-z0-9]/g, ' ').split(' ');
          tokens.forEach(t => {
            if (pathTokens.includes(t) || schema.keywords.includes(t)) score += 1;
          });
          if (score > bestScore) {
            bestScore = score;
            bestMatch = schema;
        ...
        // 3. Extrapolate C-Template and C-Var
        let template = '%d';
        let vars = bestMatch && bestScore > 0 ? bestMatch.path : 'state.unknown';

        if (bestMatch && bestScore > 0) {
          if (bestMatch.type === 'float') {
            template = '%.2f';
            if (tokens.includes('lat') || tokens.includes('lon') || tokens.includes('gps')) template = '%.6f';
          } else if (bestMatch.type === 'bool') {
            template = '%s';
            vars = `${vars} ? "ON" : "OFF"`;
          } else if (bestMatch.type === 'string') {
            template = '%s';
          }
        }

        // Auto-append generic units based on label heuristics
        if ([2, 4, 5, 6].includes(typeId)) {
          if (tokens.includes('voltage') || tokens.includes('v')) template += ',V';
          else if (tokens.includes('current') || tokens.includes('a')) template += ',A';
          else if (tokens.includes('temp') || tokens.includes('temperature')) template += ',C';
        }

        // Force dropdown formatting if Type 7
        if (typeId === 7) template = '%d,Opt1,Opt2,Opt3';
                  ...
                  {activeAIPaste === sec.id && (
                    <div className="p-4 bg-[#0a192f] border-b border-blue-900/50 flex flex-col gap-3 animate-pulse-once">
                      <div className="flex justify-between items-center">
                        <span className="text-xs text-blue-400 font-bold tracking-widest">AGENT BULK IMPORT</span>
                        <span className="text-[10px] text-blue-500 font-mono">Format: [Type], [Label] (e.g., 2, Lantern V)</span>
                      </div>
                      <textarea
                        id={`ai-paste-${sec.id}`}
                        className="w-full bg-[#020c1b] border border-blue-900/50 rounded-lg p-3 text-xs text-blue-300 font-mono h-32 outline-none focus:border-blue-500 resize-y placeholder-blue-900/50"
                        placeholder={"1, Board S/N\n2, Firmware Ver\n2, System Temp\n7, Lantern Monitor Mode"}
                      ></textarea>
                      <button
                        onClick={() => processAIImport(sec.id)}
                        className="bg-blue-600 hover:bg-blue-500 text-white font-bold py-2.5 rounded-lg text-xs tracking-widest transition-colors active:scale-[0.98]"
                      >
                        GENERATE ROWS
                      </button>
                    </div>
